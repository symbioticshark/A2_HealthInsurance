...
cd "$ROOT_DIR" || exit 1
VENV_PY="$ROOT_DIR/config/.venv/bin/python"
...
if [ -x "$VENV_PY" ] && valid_python "$VENV_PY"; then
    ...
    "$BOOTSTRAP_PY" -m venv --clear "$ROOT_DIR/config/.venv" || {
...
"$VENV_PY" -m pip install --disable-pip-version-check --quiet -r "$ROOT_DIR/config/requirements.txt" || {
