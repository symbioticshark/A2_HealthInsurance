...
def _ensure_profile():
    ...
    if first_launch:
        ...
        api_key = getpass.getpass("OpenRouter API key (press Enter to skip): ").strip()
        if api_key:
...
def _run_v1_v2_comparison():
    ...
    print("  1. Run V2 and V1 fresh (two paid batteries)")
    print("  2. Reuse a validated V2 battery, then run V1 only")
    print("  B. Back")
    choice = _prompt_choice("Select: ", {"1", "2", "B"})
    if choice == "B":
        return
    argv = ["--model", model, "--run-both" if choice == "1" else "--choose-v2-run-log"]
    original_version = tools.TOOL_INTERFACE_VERSION
    try:
...
def interactive_menu():
    ...
    while True:
        ...
        print("  B. Run evaluation set (scripted)")
        print("  C. Run guardrail checklist (scripted)")
        print("  D. Run live evaluation")
        print("  E. Compare V1 and V2 on one live model")
        print("  F. View latest result")
        print("  G. Compare results")
        print("  H. Change tester, API key, model, or default interface")
        print("  Q. Exit")
        try:
            choice = _prompt_choice("Select: ", set("ABCDEFGHQ"))
            if choice == "Q":
                print("Goodbye.")
                return
            if choice == "A":
