...
from .tools_v1 import get_claim_history as get_claim_history_v1
...
# ---------------------------------------------------------------------------
# 6. get_claim_history -- versioned D2(b) interface
# ---------------------------------------------------------------------------
# The two descriptor contracts and implementations deliberately live in
# tools_v1.py and tools_v2.py.  This module keeps the stable public aliases
# and registry so existing menu, CLI, scripted, and live callers do not
# change.  `get_claim_history` remains the final (V2) interface by default.
get_claim_history = get_claim_history_v2
...
def configure_tool_interface(version: str):
    ...
    TOOL_REGISTRY["get_claim_history"] = (
        get_claim_history_v1 if normalized == "v1" else get_claim_history_v2
    )
